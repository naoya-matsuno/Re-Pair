...
class OutputData {
    private:
        ...
        std::string delimiter_string; // データの区切りの設定
    ...
    public:
        // output_file_name: 出力ファイル名（拡張子抜き）, output_file_extension: 出力ファイルの拡張子, directory_path: 出力ファイルを保存するディレクトリのパス, delimiter_string: データの区切り文字
        OutputData(const std::string &output_file_name, const std::string &output_file_extension, const std::string &directory_path, const std::string &delimiter_string);
        ...
        // delimiter_stringを変更
        void set_delimiter_string(const std::string &delimiter_string);

        // std::ios::trunc 上書き, std::ios::app 末尾に追加 
        void output(const std::ios_base::openmode &mode, const bool &is_header) const;
};
